...
public class NewAutonomousRoutines {
	...
	public static Runnable TwoGearSideRight = new Runnable() {
		...
		//	new AutonDriveStraight(840, -0.95, 0).run();//840
			new AutonStraightMeasurement(8.5,0.95,0).run();
			// new AutonDriveForMeasurement(driveTrain, 5.2, 0.75).run();

			driveTrain.stopMotors();

			driveTrain.turnTo(305);// 305
			gch.scoringPosition();
			try {
				Thread.sleep(500);
	...
	//		new AutonDriveStraight(608, 0.85).run();// back out of peg
			new AutonStraightMeasurement((1/12)*(SmartDashboard.getNumber("DB/Slider 0", 2) * (1/0.57)), -0.85, 305);
			...
			driveTrain.turning = false;
			driveTrain.stopMotors();
			new AutonStraightMeasurement(3, -0.95, 120).run();
			...
			gca.setAngle(75);
			new AutonStraightMeasurement(9.5, 0.95, 0).run(); //1321
	...
	public static Runnable TwoGearLeftNew = new Runnable() {
		...
		public void run() {
			...
			new AutonDriveStraight(450, -0.6, startingAngle).run(); // slow down
																	// so we
																	// dont
																	// crash
																	// that hard
			...
			new DriveSemiCircle(driveTrain, SmartDashboard.getNumber("DB/Slider 0", -30), 180).run(); // semicircle
																										// to
																										// the
																										// gear
			driveTrain.stopMotors();
			...
			driveTrain.turning = false;
			driveTrain.stopMotors();
			new AutonDriveStraight(973-50, -0.9, 290).run();
			driveTrain.stopMotors();
			driveTrain.turnTo(0);
			try {
				Thread.sleep(350);
		...
		//	new AutonDriveStraight(400, 0.95).run();
			gca.setAngle(0);
			new DriveSemiCircle(driveTrain,-90,180).run();
			gca.setAngle(90);
			driveTrain.turnTo(180);
			try {
				Thread.sleep(500);
