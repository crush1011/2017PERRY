...
public class AutonomousRoutine {
	...
	public static Runnable TwoGearRight = new Runnable() {
		...
		public void run() {
			...
			driveTrain.stopMotors();
			new DriveSemiCircle(driveTrain, SmartDashboard.getNumber("DB/Slider 0", 30) * 2, 90).run();
			gca.setAngle(75);
			gch.toggleAngle(false);
			gch.toggleClamp(true);
			driveTrain.turnTo(0);
	...
	public static Runnable TwoGearLeft = new Runnable() {
		...
		public void run() {
			...
			new DriveSemiCircle(driveTrain, SmartDashboard.getNumber("DB/Slider 0", -30), 180).run(); // semicircle to the gear
			driveTrain.stopMotors();
			/*driveTrain.turnTo(180);
			gca.setAngle(Resources.getInstance().COLLECTING_POSITION);
			gch.collectingPosition();
			...
			driveTrain.stopMotors();
			new DriveSemiCircle(driveTrain, SmartDashboard.getNumber("DB/Slider 0", -30) * 1, 180).run();
			gca.setAngle(90);
			gch.toggleAngle(true);
			gch.toggleClamp(true);
			driveTrain.turnTo(0);
			...
			//driveTrain.stopMotors();
			double d = visionLoop.distanceFromTarget()+5; //10
			if(d <150){
